  ...
  try {
    ...
    if (!user) {
      ...
      const mailOption = {
        ...
        to: user.email,
        subject: "Hello ✔",
        text: `click on the following link to verify http://127.0.0.1:4000/api/v1/users/verificationToken/${token}`,
...
const ResetPasswordUser = async (req, res) => {
  //collect from param
  //collect password from body - the new password
  // ham user ko do basis pe find kareng   1. token   2. ki valid time hai ya nhi
  //empty the reset token field and expiry time field and save the password
  try {
    //res and req fucntions
    function errorResponse(err) {
      return res.status(400).json({
    ...
    return errorResponse("error occurred while resetting password");
  }
};
const ForgotPasswordUser = async (req, res) => {
  ...
  try {
    ...
    user.ResetPwdExpires = new Date(Date.now() + 10 * 60 * 1000); // 10 minutes
    await user.save();
    ...
    const mailOption = {
      ...
      text: `Click on the following link to reset your password: http://127.0.0.1:4000/api/v1/users/resetPassword/${resetToken}`,
    };
    ...
    return errorResponse("error occurred while processing request");
  }
};
